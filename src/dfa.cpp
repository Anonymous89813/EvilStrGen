...
// this function to each combination (see two paragraphs above).
    ...
    inline bool DFA::InlinedSearchLoop(SearchParams* params) {
        ...
        while (p != ep) {
            ...
            //std::cout << *s->inst_<< "--" << c  << "--"<< *ns->inst_ << std::endl;
            s = ns;
            if (s->IsMatch()) {
                matched = true;
...
// Build out all states in DFA for kind.  Returns number of states.
    ...
    void DFA::RuneToStringFile(std::deque<int> Rune_q) {
        std::ofstream OutFile;
        OutFile.open("test-uft8.txt");
        for (auto it = Rune_q.begin(); it != Rune_q.end(); it++){
            if (*it < Tx && *it > T1){
                OutFile << *it;
            }
            else if (*it >= T2 && *it < T3){
                auto Rune = (T2 ^ *it) << Bitx;
                it++;
                if (it == Rune_q.end())
                    break;
                auto Rune1 = Rune ^ (Tx ^ *it);
                OutFile << "\\u" << std::hex << Rune1;
            }
            else if (*it >= T3 && *it < T4){
                auto Rune = (T3 ^ *it) << 12;
                it++;
                if (it == Rune_q.end())
                    break;
                auto Rune1 =(Tx^ *it) << Bitx;
                it++;
                if (it == Rune_q.end())
                    break;
                auto Rune2 = Tx ^ *it;
                auto Rune3 = Rune ^ Rune1 ^ Rune2;
                OutFile << "\\u" << std::hex << Rune3;
            }
            else if (*it >= T4 && *it < T5){
                auto Rune = (T4 ^ *it) << 18;
                it++;
                ...
                if (it == Rune_q.end())
                    break;
                auto Rune3 = (Tx ^ *it);
                auto Rune4 = Rune ^ Rune1 ^ Rune2 ^ Rune3;
                OutFile << "\\\\x{" << std::hex << Rune4<< "}";
            }
        }
        OutFile.close();
    }
    ...
    vector<string> Prog::Pretreatment(std::string regex){
        vector<string> ALL_Regexes;
        std::string mid;
        std::vector<char> string_vector(regex.begin(), regex.end());
        for (auto it = string_vector.rbegin(); it != string_vector.rend(); it++) {
            if ((*it == '*' || *it == '+') && *(it + 1) != '\\'){
                ...
                if (*(it - 1) == '?') {
                    ...
                    IT = it + 1;
                auto mid1 =  "{800}" + mid;
                //std::vector<char> regex_input(mid.begin(), mid.end());
                for (; IT != string_vector.rend(); IT++){
                    mid1.insert(mid1.begin(), *IT);
                }
                ALL_Regexes.emplace_back(mid1);
            }
            mid.insert(mid.begin(), *it);
        }
        return ALL_Regexes;
    }


    bool Prog::EvilStrGen(MatchKind MatchKind, Strategy kind, Engine_Type engineType, int PathLength, std::string regex, int regex_id, int restart_times, std::string ReDoS_file){
        // aim to backtracking
        switch (engineType) {
            case Backtracking:
            case Java:
