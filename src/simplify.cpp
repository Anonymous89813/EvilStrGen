...
// The child_args are simplified Regexp*s.
    ...
    private:
        ...
        // Creates a concatenation of two Regexp, consuming refs to re1 and src.
...
// Caller must Decref() return value when done with it.
    ...
    Regexp* Regexp::SimplifyCouting(Regexp *re, int max, int min, Regexp::ParseFlags parse_flags){
        SimplifyWalker sw;
        return sw.SimplifyRepeat(re, min, max, parse_flags);
    }
...
// but in the Regexp* representation, both (x) are marked as $1.
    Regexp* SimplifyWalker::SimplifyRepeat(Regexp* re, int min, int max,
       ...
       /* if (max == min) {
            Regexp* suf = re->Incref();
            for (int i = 0; i < max; i++)
                suf = Concat2(re->Incref(), suf, f);
            nre = suf;
        }*/
