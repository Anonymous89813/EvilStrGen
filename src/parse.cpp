...
#include "src/unicode_casefold.h"
#include "src/unicode_groups.h"
#include "src/walker-inl.h"
...
// Returns NULL on error.
    Regexp* Regexp::Parse(const StringPiece& s, ParseFlags global_flags,
        ...
        int stack_size = 0; //size of stacktop_
...
/*
    Regexp* Regexp::Compute_first_pos(Regexp* re) {
        switch (re->op()) {
            default:
                break;
            case kRegexpEndText:
            ...
            case kRegexpConcat:
                for (int i = 0; i < re->nsub(); i++)
                    Compute_first_pos(re->sub()[i]);
                if (re->nsub() > 2){
                    int i = 0, k = 0, len_sub = re->nsub();
                    Regexp* re_concat = new Regexp(kRegexpConcat, PerlX);
                    //Regexp* re_concat;
                    Regexp* ac = new Regexp(kRegexpConcat, PerlX);
                    ac->AllocSub(2);
                    //re_concat->op_ =  kRegexpConcat;
                    while (len_sub > 2){
                        if (len_sub == re->nsub()){
                            re_concat->AllocSub(2);
                            //re_concat->submany_[0] = new Regexp();
                            re_concat->submany_[0] = re->submany_[i++];
                            re_concat->submany_[1] = re->submany_[i++];
                        }
                        else{
                            Regexp* re_concat1 = new Regexp(kRegexpConcat, PerlX);
                            re_concat1->AllocSub(2);
                            re_concat1->submany_[0] = re_concat;
                            re_concat1->submany_[1] = re->submany_[i++];
                            re_concat = re_concat1;
                        }
                        len_sub--;
                    }
                    ac->submany_[1] = re->sub()[re->nsub()-1];
                    re->AllocSub(2);
                    ac->submany_[0] = re_concat;
                    //ac->submany_[1] = re->sub()[re->nsub()-1];
                    re->submany_ = ac->submany_;
                }
                break;
            case kRegexpAlternate:
                for (int i = 0; i < re->nsub(); i++)
                    Compute_concat(re->sub()[i]);
                if (re->nsub() > 2){
                    int i = 0, k = 0, len_sub = re->nsub();
                    Regexp* re_concat = new Regexp(kRegexpAlternate, PerlX);
                    //Regexp* re_concat;
                    Regexp* ac = new Regexp(kRegexpAlternate, PerlX);
                    ac->AllocSub(2);
                    //re_concat->op_ =  kRegexpConcat;
                    while (len_sub > 2){
                        if (len_sub == re->nsub()){
                            ...
                            Regexp* re_concat1 = new Regexp(kRegexpAlternate, PerlX);
                            re_concat1->AllocSub(2);
                            re_concat1->submany_[0] = re_concat;
                            re_concat1->submany_[1] = re->submany_[i++];
                            re_concat = re_concat1;
            ...
            case kRegexpRepeat:
                ...
                Compute_concat(re->sub()[0], s);
                re = re->SimplifyCouting(re->sub()[0], re->max(), re->min(), PerlX);
...
*/
    Regexp* Regexp::Compute_concat(Regexp* re, std::string* s) {
        switch (re->op()) {
            default:
                break;
            case kRegexpEndText:
