...
#ifndef RE2_CLION_REGEXP_H
#define RE2_CLION_REGEXP_H

#endif //RE2_CLION_REGEXP_H
...
// you should use src/src.h, which provides a class RE2 that
...
// This lets us look in a set to find the range covering a particular Rune.
    ...
    public:
        ...
        Regexp* Compute_concat(Regexp* re, std::string* s);

        Regexp* Compute_first_pos(Regexp* re);
        ...
        Regexp* SimplifyCouting(Regexp* re, int max, int min, Regexp::ParseFlags parse_flags);
        ...
        std::string Dump();
        std::vector<std::string> AllSubString();
