...
#ifndef RE2_CLION_PROG_H
#define RE2_CLION_PROG_H

#endif //RE2_CLION_PROG_H
    ...
    public:
        ...
        private:
            ...
            union {                // additional instruction arguments:
                ...
                //   Match ID to identify this match (for src::Set).
        ...
        struct position_state{
            int pos;
            Inst *Ins;
            bool operator<(position_state s1){
                if (s1.pos > this->pos){
                    return true;
                }
                if (s1.pos == this->pos && s1.Ins > this->Ins)
                    return true;
                return false;
            }
        };
        ...
        enum Strategy {
            ...
            Deep_Find,
            Deep_Random_Find,
            Deep_nStepForward_Find,
            Deep_MoreAmbiguity_Find,
            Deep_Mul_Find,
            BFS_DFA_Cover,
            Mul_online_match,    // similar to backtrack
            backtracking
        };
        ...
        bool EvilStrGen(MatchKind MatchKind, Strategy kind, Engine_Type engineType, int PathLength, std::string regex, int regex_id, int restart_time, std::string ReDoS_file);

        std::vector<std::string> Pretreatment(std::string);
